  ...
  <body>
	<?php include template("index","header"); ?> 
		<div class="main_left">
			...
			<div class="operate"><span id="favorite"><a href="javascript:;" onclick="add_favorite('<?php echo $title;?>');">收藏</a></span>  | <a href="javascript:;" onClick="window.print();" class="print">打印</a></div>
	        <div class="pn">		
			...
			<div class="tuijian">
			<ul>	
			<?php $tag = yzm_base::load_sys_class('yzm_tag');if(method_exists($tag, 'lists')) {$data = $tag->lists(array('field'=>'title,url,thumb','modelid'=>$modelid,'order'=>'RAND()','limit'=>'4',));}?>
		...
		<div class="main_right">		 
			...
			<?php $tag = yzm_base::load_sys_class('yzm_tag');if(method_exists($tag, 'hits')) {$data = $tag->hits(array('field'=>'title,url,thumb','modelid'=>$modelid,'limit'=>'5',));}?>
			<?php if(is_array($data)) foreach($data as $v) { ?>	
			...
			<div class="l_list">
			<ul>
			<?php $tag = yzm_base::load_sys_class('yzm_tag');if(method_exists($tag, 'comment_ranking')) {$data = $tag->comment_ranking(array('modelid'=>'1','limit'=>'10',));}?>
			<?php if(is_array($data)) foreach($data as $v) { ?>	

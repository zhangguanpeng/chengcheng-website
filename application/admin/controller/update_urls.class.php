...
class update_urls extends common {
	...
	public function change_model() {
		...
		$modelinfo = get_modelinfo();
 		if($modelid){
			$catinfo = D('category')->field('catid,catname,type')->where(array('modelid'=>$modelid))->select();
		}else{
			$catinfo = get_category();
		}
	...
	public function update_category_url() {
 		if(isset($_POST['dosubmit'])) {
			...
			foreach($catinfo as $val){
				...
				//如果系统设置是伪静态模式
				if($url_rule){
					$pclink = URL_MODEL == 1 ? SITE_URL.'index.php/'.$val['catdir'].'/' : SITE_URL.$val['catdir'].'/';
				}else{  
	...
	private function get_url($url_rule, $catid, $id){
		...
		if($url_rule){
			...
			$url = URL_MODEL == 1 ? SITE_URL.'index.php/'.$catinfo['catdir'].'/'.$id.'.html' : SITE_URL.$catinfo['catdir'].'/'.$id.'.html';
		}else{  

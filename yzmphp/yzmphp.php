...
yzm_base::load_common('data/version.php');  //用户自定义加载文件
	...
	public static function load_common($path, $m = '') {
		if(!$m){
			if (is_file(YZMPHP_PATH.'common'.DIRECTORY_SEPARATOR.$path)) {
				include YZMPHP_PATH.'common'.DIRECTORY_SEPARATOR.$path;
			}else{
			...
			if (is_file(APP_PATH.$m.DIRECTORY_SEPARATOR.'common'.DIRECTORY_SEPARATOR.$path)) {
				include APP_PATH.$m.DIRECTORY_SEPARATOR.'common'.DIRECTORY_SEPARATOR.$path;
			}else{

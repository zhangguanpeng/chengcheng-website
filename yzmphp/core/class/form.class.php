...
class form {
	 ...
	 * @param $style 样式
	 */
	public static function image($name, $val = '', $style = 'width:370px') {		
		$string = '<input class="input-text uploadfile" type="text" name="'.$name.'"  value="'.$val.'"  onMouseOver="yzm_img_preview(\''.$name.'\', this.value)" id="'.$name.'" style="'.$style.'" > <a href="javascript:;" onclick="yzm_upload_att(\''.U('attachment/api/upload_box', array('module'=>ROUTE_M, 'pid'=>$name)).'\')" class="btn btn-primary radius upload-btn"><i class="Hui-iconfont">&#xe642;</i> 浏览文件</a>';
	...
	public static function images($name, $val = '', $n = 20) {
		...
		if($val){
			...
			foreach($arr as $key => $val){
				$string .= '<li>文件：<input type="text" name="'.$name.'[url][]" value="'.$val['url'].'" id="'.$name.'_'.$key.'" onMouseOver="yzm_img_preview(\''.$name.'_'.$key.'\', this.value)" class="input-text w_300"> 描述：<input type="text" name="'.$name.'[alt][]" value="'.$val['alt'].'" class="input-text w_200"><a href="javascript:;" onclick="remove_li(this);">删除</a></li>';
			}
		}					
		$string .= 	'</ul></fieldset>
	 ...
	 * @param $style 样式
	 */
	public static function editor($name = 'content', $val = '', $style='width:100%;height:400px') {	
		$string = '';
		if(!defined('EDITOR')) {
			define('EDITOR', 1);
	 ...
	 * @param $style 样式
	 */
	public static function editor_mini($name = 'content', $val = '', $style='width:100%;height:400px') {		
		$string = '<script type="text/javascript" charset="utf-8" src="'.STATIC_URL.'plugin/ueditor/1.4.3.3/ueditor.config.js"></script>
			<script type="text/javascript" charset="utf-8" src="'.STATIC_URL.'plugin/ueditor/1.4.3.3/ueditor.all.min.js"> </script>
			<script type="text/javascript" charset="utf-8" src="'.STATIC_URL.'plugin/ueditor/1.4.3.3/lang/zh-cn/zh-cn.js"></script>
			<script id="'.$name.'" type="text/plain" style="'.$style.'" name="'.$name.'">'.$val.'</script>

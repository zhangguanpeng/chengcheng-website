...
class db_mysql{
	...
	public static function connect(){
		self::$link = @mysql_connect(C('db_host'), C('db_user'), C('db_pwd'));  	
		if(self::$link == false) application::halt("Can not connect to MySQL server!");        
		$db = mysql_select_db(C('db_name'), self::$link);            	
	...
	private function geterr($msg = ''){
		if(APP_DEBUG){
			echo '<div style="font-size:14px;text-align:left; border:1px solid #9cc9e0;line-height:25px; padding:5px 10px;color:#000;font-family:Arial, Helvetica,sans-serif;"><b> Error : </b>'. $msg .' <br /><b> MySQL Errno : </b>'. mysql_errno() .' <br /> <b>MySQL Error : </b> <span>'. mysql_error() .'</span> <br /><a href="http://www.yzmcms.com/" target="_blank" style="color:red">Need Help?</a></div>';
			exit;
		}else{
			error_log('<?php exit;?> MySQL Error: '.date('m-d H:i:s').' | Errno: '.mysql_errno().' | Error: '.mysql_error().' | SQL: '.$msg."\r\n", 3, YZMPHP_PATH.'cache/error_log.php');
